 ...
 * @author donia
 */
public class MetierListForm extends BaseForm {
    
    List<Metier> prods = ServiceMetier.getInstance().affichageMetiers();
    int sum = 0;

    Container all = new Container(new BoxLayout(BoxLayout.Y_AXIS));
    public MetierListForm(Resources res) {
        super(BoxLayout.y());
...
//        profilePicLabel.setMask(mask.createMask());
        ...
        Container remainingTasks = BoxLayout.encloseY(
                ...
                new Label("Metiers", "CenterSubTitle")
        );
        ...
        fab.addActionListener((e) -> new AddMetierForm(res));
        fab.getAllStyles().setMarginUnit(Style.UNIT_TYPE_PIXELS);
        fab.getAllStyles().setMargin(BOTTOM, completedTasks.getPreferredH() - fab.getPreferredH() / 2);
        tb.setTitleComponent(fab.bindFabToContainer(titleCmp, CENTER, BOTTOM));
        add(new Label("List Metiers", "TodayTitle"));
        ...
        searchField.addActionListener(e -> {
            ...
            List<Metier> filteredData = new ArrayList<Metier>();
            for (Metier p : prods) {
    ...
    private void addButtonBottom(Resources res,Metier p, int color, boolean first) {
        MultiButton finishLandingPage = new MultiButton(p.getNom());
        finishLandingPage.setTextLine2(p.getType());
        finishLandingPage.setUIID("Container");
        finishLandingPage.setUIIDLine1("TodayEntry");
        finishLandingPage.addActionListener((e) -> Dialog.show(null, p.getDescription(), "OK", null));
        ...
        suppbtn.addActionListener((e)->{
            ServiceMetier.deleteMetier(p.getId());
            new MetierListForm(res).show();
        });
        modbtn.addActionListener((e)->
                {
        AddMetierForm apf = new AddMetierForm(res);
           apf.infalteUI(p);
           apf.show();}
        );
